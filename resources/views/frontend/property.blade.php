                    ...
                    <div class="col-lg-4 mb-4">
                        <div class="card border-0 shadow-sm">
                            <span style="border-radius: 0 0.4rem 0 0.4rem"
                                class="text-white px-4 py-1 end-0 {{ $property->category->name === 'For Sale' ? 'bg-info' : 'bg-warning' }} position-absolute">{{ $property->category->name }}</span>
                            <img src="{{ Storage::url($property->galleries()->first()->path) }}" height="310"
                                style="object-fit: cover" class="card-img-top" alt="{{ $property->name }}" />
                            <div class="card-body">
                                <h5 class="card-title">
                                    {{ $property->name }}
                                </h5>
