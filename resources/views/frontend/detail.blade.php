    ...
    <main class="mt-5">
        <section class="container detail-properties">
            <div class="text-center mb-4">
                <h2>{{ $property->name }}</h2>
                <i class="uil uil-map-marker text-secondary"></i><span>{{ $property->location }}</span>
            </div>
            <div class="row mb-4">
                <div class="col-lg-6 mb-4">
                    <img height="460" style="object-fit: cover" class="rounded w-100"
                        src="{{ Storage::url($property->galleries()->latest()->first()->path) }}" alt="" />
                </div>
                <div class="col-lg-6">
                    <div class="row">
                        @foreach ($property->galleries->take(4) as $gallery)
                            <div class="col-lg-6 mb-4">
                                <img style="object-fit: cover" class="img-fluid h-100 w-100 rounded"
                                    src="{{ Storage::url($gallery->path) }}" alt="" />
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="col-lg-4">
                    <div class="card py-5 h-100 border-0">
                        <div class="mx-auto text-center">
                            <img width="110" height="110" style="object-fit: cover"
                        ...
                        <a href="https://wa.me/{{ preg_replace('/[^\d]/', '', $property->agent->phone) }}"
                            class="w-75 p-3 text-center mx-auto btn btn-primary">Get Property</a>
                    </div>
                </div>
            </div>
        </section>
